...
export function listMovies(db: Database.Database, params: {
  ...
  // When searching: include boxset children so individual films appear in results.
  // Without search: top-level only (boxset children are shown under their parent).
  const topLevelOnly = q ? '' : ' AND boxset_parent_id IS NULL'
  let listWhere  = `is_deleted = 0 AND in_collection = 1${topLevelOnly} AND (is_boxset IS NULL OR is_boxset = 0)`
  let countWhere = `is_deleted = 0 AND in_collection = 1${topLevelOnly} AND (is_boxset IS NULL OR is_boxset = 0)`
