...
function runMigrations(instance: Database.Database = db): void {
  ...
  try {
    ...
    if (!done) {
      instance.exec(`
        UPDATE movies SET boxset_parent_remote_id = boxset_parent_id
        WHERE boxset_parent_id IS NOT NULL AND boxset_parent_remote_id IS NULL;
        ...
        WHERE boxset_parent_remote_id IS NOT NULL;
      `)
      instance.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('boxset_parent_repaired', '1')").run()
    }
  } catch (e) {
    console.error('Boxset-Reparatur fehlgeschlagen:', e)
  }
