<template>
  <div v-if="list" class="p-8">
    ...
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center gap-4">
        <router-link to="/lists" class="w-9 h-9 flex items-center justify-center rounded-xl glass hover:border-red-500/50 transition-colors text-[var(--text-muted)]">
    ...
    <div v-else-if="list.items.length > 0" class="grid grid-cols-[repeat(auto-fill,minmax(160px,1fr))] gap-6">
      ...
      >
        <div class="relative aspect-[2/3] rounded-2xl overflow-hidden glass group-hover:border-red-500/50 group-hover:scale-105 transition-all duration-300 shadow-[var(--shadow-main)]">
          <img
  ...
  <div v-if="showAddModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" @click.self="closeAddModal">
    <div class="glass rounded-2xl p-6 w-full max-w-lg shadow-2xl">
      ...
      <input
        ...
        class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-[var(--text-main)] placeholder-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-red-500/50 transition-colors mb-3"
        autofocus
      />
      ...
      <div class="max-h-80 overflow-y-auto space-y-1 custom-scrollbar">
        ...
        >
          <div class="w-8 aspect-[2/3] rounded-lg overflow-hidden bg-white/5 border border-white/10 flex-shrink-0">
            <img
              v-if="resolveMediaUrl(movie.cover_url || movie.cover_path, movie.remote_id ?? undefined)"
              :src="resolveMediaUrl(movie.cover_url || movie.cover_path, movie.remote_id ?? undefined)!"
              :alt="movie.title"
